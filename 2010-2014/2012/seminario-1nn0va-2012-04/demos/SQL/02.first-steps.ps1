...
# we can filter it
dir JobServer\Jobs | ? { $_.LastRunOutcome -eq "failed" }
...
#let's try
dir Databases | ? { $_.RecoveryModel -eq "Full" }
...
dir Databases | ? { $_.RecoveryModel -eq 1 }
dir Databases | ? { $_.RecoveryModel.ToString() -eq "Full" }

# or we can use enumerator's values
# cd SQLSERVER:\SQL\M6500\PROD1
dir Databases | ? { $_.RecoveryModel -eq [Microsoft.SqlServer.Management.Smo.RecoveryModel]::Full }
...
dir SQLSERVER:\SQL\M6500\PROD1\JobServer\Jobs | ? { $_.LastRunOutcome -eq "Failed" };
...
$SQLInstance.JobServer.Jobs | ? { $_.LastRunOutcome -eq "Failed" };

#Example 3 using SQLPSX 
#get-module -listAvailable
#import-module Agent;
Import-Module SQLPSX
Get-AgentJob "localhost\prod1" | ? { $_.LastRunOutcome -eq "Failed" };
