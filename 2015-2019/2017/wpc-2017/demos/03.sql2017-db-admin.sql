...
SELECT * FROM sys.dm_os_enumerate_filesystem('C:\SQLServer\MSSQL14.SQL2017\MSSQL\Backup', '*.bak');
SELECT * FROM sys.dm_os_file_exists('C:\SQLServer\MSSQL14.SQL2017\MSSQL\Log');
SELECT * FROM sys.dm_os_file_exists('C:\SQLServer\MSSQL14.SQL2017\MSSQL\Log\log.trc');
GO
...
USE master;
GO
IF EXISTS (SELECT * FROM sys.databases WHERE [name] = 'WideWorldImporters_Copy') DROP DATABASE WideWorldImporters_Copy;
GO

-- clone all: schema, statistics and query store
DBCC CLONEDATABASE('WideWorldImporters', 'WideWorldImporters_Copy');
GO

-- Metadata to check if database is a clone
SELECT DATABASEPROPERTYEX('WideWorldImporters_Copy','isClone')
GO

-- files re-created with unique physical names
SELECT * FROM WideWorldImporters.sys.database_files;
SELECT * FROM WideWorldImporters_Copy.sys.database_files;
GO

DROP DATABASE WideWorldImporters_Copy;
GO
...
SELECT * FROM sys.dm_exec_query_statistics_xml(63);
SELECT * FROM sys.dm_exec_query_profiles WHERE session_id = 63;
GO
