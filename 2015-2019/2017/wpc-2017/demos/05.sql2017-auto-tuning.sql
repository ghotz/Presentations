------------------------------------------------------------------------
-- Script:		sql2017-auto-tuning.sql
...
-- Helper procedures
--CREATE OR DROP PROCEDURE [dbo].[report] (@packagetypeid int)
--AS BEGIN
...
--END
--GO
--CREATE OR ALTER DROP PROCEDURE [dbo].[regression]
--AS BEGIN
...
EXEC	sp_executesql N'select avg([UnitPrice]*[Quantity]) from Sales.OrderLines where PackageTypeID = @packagetypeid', N'@packagetypeid int'
		, @packagetypeid = 7;
GO 150	-- increase as nedded if laptop very fast!
...
------------------------------------------------------------------------
-- Plan regression identification & auto tuning (doesn't work)
------------------------------------------------------------------------
...
EXEC	sp_executesql N'select avg([UnitPrice]*[Quantity]) from Sales.OrderLines where PackageTypeID = @packagetypeid', N'@packagetypeid int'
		, @packagetypeid = 7;
GO 250	-- increase as nedded if laptop very fast!
